...
public class UserControllerTest extends AbstractTransactionalTestNGSpringContextTests {
    ...
    void setup()
    {
	    ...
	    void getUserIsAdmin() throws Exception
	    {
	    	...
	    	header.add(HttpHeaders.AUTHORIZATION, "Bearer " + token);
	    	 ...
	    	 this.mockMvc.perform( get( "/users/2" ).headers(header) )
	   ...
	   @Test
	    void getUserIsUser() throws Exception
	    {
	    	...
	    	rHeader.add(HttpHeaders.AUTHORIZATION, "Bearer " + token);
	    	 ...
	    	 this.mockMvc.perform( get( "/users/2" ).headers(rHeader) )
	          .andExpect( status().isOk() )
	          .andExpect( jsonPath( "username" ).value( "ammar" ) );
	    }
	    ...
	    void getUserIsNotFound() throws Exception
	    {
	    	...
	    	header.add(HttpHeaders.AUTHORIZATION, "Bearer " + token);
	    	 this.mockMvc.perform( get( "/users/90" ).headers(header) )
	            .andExpect( status().is(Matchers.is(404)) );
	    }
        ...
        void getUsersIsAdmin() throws Exception
        {
            ...
            this.mockMvc.perform( get( "/users" ).headers(header) )
                    .andExpect( status().isOk() )
    ...
    void createUserPass() throws Exception
    {
        ...
        header.add(HttpHeaders.AUTHORIZATION, "Bearer " + token);
        this.mockMvc.perform( post( "/users" ).headers(header).contentType(
                MediaType.APPLICATION_JSON_UTF8).content("{\"username\" : \"bob\"," +
                                                        "\"password\" : \"abcd\"," +
                                                        "\"firstName\" : \"Bob\"," +
                                                        "\"lastName\" : \"Bobby\"," +
                                                        "\"email\": \"bob@localhost.domain\"}") )
    ...
    void editUserIsAdmin() throws Exception
    {
        ...
        header.add(HttpHeaders.AUTHORIZATION, "Bearer " + token);
        this.mockMvc.perform( put( "/users/2" ).headers(header).contentType(
